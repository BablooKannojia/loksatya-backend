...
const findSliderItem = async (id) => {
  let item = await Article.findById(id);
  if (item) return { item, Model: Article, contentType: "article" };

  item = await LiveNews.findById(id);
  if (item) return { item, Model: LiveNews, contentType: "liveNews" };

  return { item: null, Model: null, contentType: null };
};
...
const addSliderOrder = async (req, res) => {
  try {
    ...
    if (!id || !sliderOrder) {
      return res.status(400).json({ success: false, message: "id and sliderOrder required" });
    }

    if (sliderOrder < 1 || sliderOrder > 4) {
      return res.status(400).json({ success: false, message: "Slider position must be between 1-4" });
    }

    const { item, contentType } = await findSliderItem(id);

    if (!item) {
      return res.status(404).json({ success: false, message: "Item not found" });
    }

    if (typeof item.sliderOrder === "number") {
      return res.status(400).json({ success: false, message: "Already in slider" });
    }

    // Position 4 pe jo bhi ho, hatao
    await Article.findOneAndUpdate({ sliderOrder: 4 }, { $unset: { sliderOrder: "" } });
    await LiveNews.findOneAndUpdate({ sliderOrder: 4 }, { $unset: { sliderOrder: "" } });

    await Article.updateMany(
      { sliderOrder: { $gte: sliderOrder, $lte: 3 } },
      { $inc: { sliderOrder: 1 } }
    );
    await LiveNews.updateMany(
      { sliderOrder: { $gte: sliderOrder, $lte: 3 } },
      { $inc: { sliderOrder: 1 } }
    );

    item.sliderOrder = sliderOrder;
    await item.save();

    const articles = await Article.find({ sliderOrder: { $type: "number" } }).lean();
...
const getSliderArticles = async (req, res) => {
  try {
    ...
    const liveNews = await LiveNews.find({
      ...
      .select("_id title image slug sliderOrder category")
      .lean();

    const normalizedLiveNews = liveNews.map((item) => ({
      ...item,
      topic: item.category,
      contentType: "liveNews",
    }));
    ...
    const combined = [...normalizedArticles, ...normalizedLiveNews].sort(
      (a, b) => a.sliderOrder - b.sliderOrder
    );
...
const updateSliderOrder = async (req, res) => {
  try {
    ...
    if (!id || !sliderOrder) {
      return res.status(400).json({ success: false, message: "id and sliderOrder are required" });
    }

    if (sliderOrder < 1 || sliderOrder > 4) {
      return res.status(400).json({ success: false, message: "sliderOrder must be between 1 and 4" });
    }

    const { item: current, contentType } = await findSliderItem(id);
    ...
    // Dono collections ko ek "shared pool" ki tarah treat karna hai, isliye
    // shift-updates dono models pe chalayenge
    const shiftBothModels = async (filter, update) => {
      await Article.updateMany(filter, update);
      await LiveNews.updateMany(filter, update);
    };

    if (oldOrder == null) {
      // Position 4 pe jo bhi ho (Article ya LiveNews), usko hatao
      await Article.findOneAndUpdate({ sliderOrder: 4 }, { $unset: { sliderOrder: "" } });
      await LiveNews.findOneAndUpdate({ sliderOrder: 4 }, { $unset: { sliderOrder: "" } });

      await shiftBothModels(
        { sliderOrder: { $gte: sliderOrder, $lte: 3 } },
        { $inc: { sliderOrder: 1 } }
      );
      ...
      await current.save();
    } else if (oldOrder === sliderOrder) {
      return res.json({ success: true, message: "No changes" });
    } else if (oldOrder < sliderOrder) {
      await shiftBothModels(
        { sliderOrder: { $gt: oldOrder, $lte: sliderOrder } },
        { $inc: { sliderOrder: -1 } }
      );
      current.sliderOrder = sliderOrder;
      await current.save();
      ...
      await shiftBothModels(
        { sliderOrder: { $gte: sliderOrder, $lt: oldOrder } },
        { $inc: { sliderOrder: 1 } }
      );
      current.sliderOrder = sliderOrder;
      await current.save();
    ...
    const articles = await Article.find({ sliderOrder: { $type: "number" } })
      .select("_id title sliderOrder")
      .lean();
    const liveNews = await LiveNews.find({ sliderOrder: { $type: "number" } })
      .select("_id title sliderOrder")
      .lean();
...
const removeSlider = async (req, res) => {
  try {
    ...
    const { item, Model } = await findSliderItem(id);
    if (!item) {
      return res.status(404).json({ success: false, message: "Item not found" });
    }

    await Model.findByIdAndUpdate(id, { $unset: { sliderOrder: "" } });

    const articles = await Article.find({ sliderOrder: { $type: "number" } }).sort({ sliderOrder: 1 });
    const liveNews = await LiveNews.find({ sliderOrder: { $type: "number" } }).sort({ sliderOrder: 1 });

    const all = [...articles, ...liveNews].sort((a, b) => a.sliderOrder - b.sliderOrder);

    for (let i = 0; i < all.length; i++) {
      all[i].sliderOrder = i + 1;
      await all[i].save();
    }
